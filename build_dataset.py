...
** DEPRECATED — DO NOT RUN WITHOUT READING HANDOFF.md FIRST **
players_worldcup2026.csv (the file the live app actually uses) is no longer generated by
this script alone. After this script's automated pass, the ~36 lowest-confidence rows were
hand-validated by a person/another agent directly in the CSV (match_source="web-verified"),
and a further ~39 rows were deliberately given a capital-city fallback for born_city/lat/lng
where the true birth city is unknown (needs_review="fallback") — see HANDOFF.md.
Those corrections live ONLY in players_worldcup2026.csv, not in this script's logic. The
established workflow during development WAS "run this script, then `cp players_enriched.csv
players_worldcup2026.csv`" — do NOT do that copy step anymore, it will silently destroy the
hand corrections. This script is kept for reference / methodology only. If the roster changes
and you must regenerate data, regenerate into players_enriched.csv as before, then manually
merge only the affected rows into players_worldcup2026.csv — never overwrite the whole file.
"""
...
if __name__ == "__main__":
    if "--force" not in sys.argv:
        sys.exit(
            "build_dataset.py is deprecated — players_worldcup2026.csv now contains hand-\n"
            "validated corrections (match_source=web-verified, needs_review=fallback) that\n"
            "this script knows nothing about and does NOT reproduce. Read the big comment at\n"
            "the top of this file and HANDOFF.md before doing anything else.\n"
            "Refusing to run. Pass --force only if you have read HANDOFF.md and understand\n"
            "you must manually re-merge hand-validated rows afterward — this script will\n"
            "still only write players_enriched.csv, never players_worldcup2026.csv directly."
        )
    main()
